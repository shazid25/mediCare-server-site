...
async function run() {
  try {
    ...
    const transactionsCollection = db.collection("transactions"); // Add transactions collection
    ...
    app.post("/transactions", verifyToken, async (req, res) => {
      try {
        ...
        res.status(201).json({ 
          ...
          data: { transactionId: result.insertedId } 
        });
      } catch (error) {
    ...
    app.get("/transactions/user/my-transactions", verifyToken, async (req, res) => {
      try {
        const userEmail = req.decoded.email;
        const transactions = await transactionsCollection.find({ userEmail }).sort({ transactionDate: -1 }).toArray();
        res.send({ success: true, data: transactions });
      } catch (err) {
        console.error("GET /transactions/user/my-transactions error:", err);
